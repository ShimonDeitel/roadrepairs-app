...
struct ContentView: View {
    ...
    @State private var editingItem: Repair?
    ...
    var body: some View {
        NavigationStack {
            ...
            .themedBackground()
            .navigationTitle("Roadside Repairs")
            .toolbar {
...
struct EntryEditorView: View {
    ...
    @State private var draftRepairdescription: String = ""
    @State private var draftLocation: String = ""
    @State private var draftCost: Double = 0
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                TextField("Repairdescription", text: $draftRepairdescription)
                    .accessibilityIdentifier("field_repairDescription")
                TextField("Location", text: $draftLocation)
                    .accessibilityIdentifier("field_location")
                TextField("Cost", value: $draftCost, format: .number)
    ...
    private func loadIfEditing() {
        if case .edit(let item) = mode {
        draftRepairdescription = item.repairDescription
        draftLocation = item.location
        draftCost = item.cost
        draftDate = item.date
        draftNotes = item.notes
        } else {
        draftRepairdescription = ""
        draftLocation = ""
        draftCost = 0
        draftDate = Date()
        draftNotes = ""
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(Repair(repairDescription: draftRepairdescription, location: draftLocation, cost: draftCost, date: draftDate, notes: draftNotes))
        case .edit(let item):
            var updated = item
            updated.repairDescription = draftRepairdescription
            updated.location = draftLocation
            updated.cost = draftCost
